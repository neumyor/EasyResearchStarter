...
- **单写入者**：任何编辑、运行前准备、暂存或提交都必须由持有工作区锁的唯一 agent 完成；
  其他 agent 只能只读分析。
- **长实验监控**：预计超过 1 分钟的实验优先在后台、以可检查的日志或任务状态运行；超过
  5 分钟后至少汇报一次已运行时间、进度证据与按当前进度推算的 ETA，不能合理推算时明确说明。
...
- 关键实验后从**落盘的预测/误差产物**（CSV / npz / 图）中程序化挑选 bad case，覆盖不同
  失败模式，数量可控。
- 对结论有实质影响的生成图（包括 Matplotlib 图），若当前宿主可直接查看图片，必须在分析前
  至少视觉检查一次：确认数据、标签/单位/图例、裁切、布局和表达结论均正确。没有查看工具时，
  记录限制并做可用的非视觉检查，不得表述为已验图。
